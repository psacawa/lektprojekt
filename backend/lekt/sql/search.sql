...
--
CREATE
OR REPLACE PROCEDURE compute_lexeme_weights () AS $$
BEGIN 
...
CREATE UNLOGGED TABLE lekt_lexeme_idf AS (
  ...
  FROM
    ...
    JOIN lekt_phrase bp ON (pp.base_id = bp.phrase_id)
    JOIN lekt_phrase tp ON (pp.target_id = tp.phrase_id)
    JOIN lekt_doc_counts dc ON (
      bp.lang_id = dc.base_lang_id
      AND tp.lang_id = dc.target_lang_id
    )
    JOIN lekt_phrase_words tpw ON (tp.phrase_id = tpw.phrase_id)
    JOIN lekt_word tw ON (tpw.word_id = tw.word_id)
  WHERE
...
FROM
  ...
  JOIN lekt_lexeme_idf ON (
    lekt_lexeme_tf.base_lang_id = lekt_lexeme_idf.base_lang_id
...
--
CREATE
OR REPLACE PROCEDURE compute_feature_weights () AS $$
BEGIN
...
FROM
  ...
  JOIN lekt_feature_idf ON (
    lekt_feature_tf.base_lang_id = lekt_feature_idf.base_lang_id
...
CREATE
OR REPLACE PROCEDURE compute_observable_weights () AS $$
BEGIN
...
FROM
  ...
  JOIN lekt_observable_idf fidf ON (
    ftf.base_lang_id = fidf.base_lang_id
